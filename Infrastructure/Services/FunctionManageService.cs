    ...
    public class FunctionManageService : IFunctionManageService
    {
        ...
        public async Task<dynamic> GetPagingCore(PagingParamDefault pagingParam)
        {
            try
            {
                ...
                var query = _context.FunctionManages.Where(x => x.IsDeleted == false);
                if (!string.IsNullOrWhiteSpace(pagingParam.Content))
                {
                    var content = pagingParam.Content.Trim();
                    query = query.Where(x => x.Name.Contains(content) || x.Router.Contains(content));
                }

                var data = await query.OrderByDescending(x => x.Id).Include(x => x.UnitRole).Include(x => x.Controller).Select(x => new
                {
