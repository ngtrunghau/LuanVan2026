...
export default {
  ...
  mounted() {
    ...
    this.$refs.ref_info_modal.addEventListener('hidden.bs.modal', event => {
      this.model = unitRoleModel.baseJson()
    });
  ...
  methods: {
    async getData() {
      ...
      await this.$store.dispatch("unitRoleStore/getPagingParams", params ).then(res => {
    ...
    async handleGetInfo(id) {
      await this.$store.dispatch("unitRoleStore/getById", {id : id}).then((res) => {
        if (res != null && res.code ===0) {
          this.model = unitRoleModel.getJson(res.data);
        }
      });
    },
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("unitRoleStore/delete", { 'id': this.model.id }).then((res) => {
          if (res != null && res.code ===0) {
    ...
    async handleSubmit() {
      if (
        ...
        await this.$store.dispatch("unitRoleStore/create", this.model).then((res) => {
          if (res != null && res.code ===0) {
            this.getData();
            this.model= unitRoleModel.baseJson()
            this.theModal.hide();
