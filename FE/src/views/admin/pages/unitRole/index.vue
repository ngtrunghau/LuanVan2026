              ...
              <div class="card-header">
                ...
                <div class="top-nav-search">
                  <form @submit.prevent="handleSearch">
                    <input v-model.trim="searchText" type="text" class="form-control" placeholder="Tìm theo tên vai trò..." />
                          ...
                          <div class="modal-body">
                            ...
                            >
                              <div class="row">
                                <div class="col-12">
                                  <div class="mb-3">
                                    <label class="text-left">Tên vai trò</label>
...
async function getData() {
  ...
  await proxy.$store.dispatch("unitRoleStore/getPagingParams", params).then(res => {
...
async function handleGetInfo(id) {
  await proxy.$store.dispatch("unitRoleStore/getById", {
    id: id
  }).then(res => {
    if (res != null && res.code === 0) {
      state.model = unitRoleModel.getJson(res.data);
    }
  });
}
...
async function handleDelete() {
  if (state.model.id != 0 && state.model.id != null && state.model.id) {
    await proxy.$store.dispatch("unitRoleStore/delete", {
      'id': state.model.id
    }).then(res => {
      if (res != null && res.code === 0) {
...
async function handleSubmit() {
  if (state.model.id != 0 && state.model.id != null && state.model.id) {
    await proxy.$store.dispatch("unitRoleStore/update", state.model).then(res => {
      if (res != null && res.code === 0) {
        getData();
        state.model = unitRoleModel.baseJson();
        state.theModal.hide();
      }
      proxy.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res));
    });
  } else {
    await proxy.$store.dispatch("unitRoleStore/create", state.model).then(res => {
      if (res != null && res.code === 0) {
        getData();
        state.model = unitRoleModel.baseJson();
        state.theModal.hide();
