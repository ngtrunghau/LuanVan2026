...
async function handleGetInfo(id) {
  await proxy.$store.dispatch("loaiStore/getById", {
    ...
    if (res != null && res.code === 0) {
      state.model = loaiModel.getJson(res.data);
    }
  });
}
...
async function handleDelete() {
  if (state.model.id != 0 && state.model.id != null && state.model.id) {
    await proxy.$store.dispatch("loaiStore/delete", {
      'id': state.model.id
    }).then(res => {
      if (res != null && res.code === 0) {
...
async function handleSubmit() {
  if (state.model.id != 0 && state.model.id != null && state.model.id) {
    await proxy.$store.dispatch("loaiStore/update", state.model).then(res => {
      if (res != null && res.code === 0) {
        getData();
        state.model = loaiModel.baseJson();
        state.theModal.hide();
      }
      proxy.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res));
    });
  } else {
    await proxy.$store.dispatch("loaiStore/create", state.model).then(res => {
      if (res != null && res.code === 0) {
        getData();
        state.model = loaiModel.baseJson();
        state.theModal.hide();
