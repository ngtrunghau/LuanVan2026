                          ...
                          <div class="modal-body">
                            ...
                            >
                              <div class="row">
                                <div class="col-12">
                                  <div class="mb-3">
                                    <label class="text-left">Tên loại</label>
                                ...
                                <div class="col-12">
                                  <div class="mb-3">
                                    <label class="text-left">Sort</label>
                                    <span style="color: red">&nbsp;*</span>
                                    <Field
                                        v-model="model.sort"
                                        placeholder="Vui lòng nhập sort"
                                        name="sort"
                                        type="number"
                                        class="form-control"
                                        :class="{ 'is-invalid': errors.sort }"
                                    />
                                    <div class="invalid-feedback">{{ errors.sort }}</div>
                                  </div>
                                </div>
                              </div>
...
import { loaiModel } from "@/models/loaiModel";
...
export default {
  ...
  methods: {
    async getData() {
      ...
      await this.$store.dispatch("loaiStore/getPagingParams", params ).then(res => {
    ...
    async handleGetInfo(id) {
      await this.$store.dispatch("loaiStore/getById", {id : id}).then((res) => {
        if (res != null && res.code ===0) {
          this.model = loaiModel.getJson(res.data);
        }
      });
    },
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("loaiStore/delete", { 'id': this.model.id }).then((res) => {
          if (res != null && res.code ===0) {
    ...
    async handleSubmit() {
      if (
        ...
        await this.$store.dispatch("loaiStore/update", this.model).then((res) => {
          if (res != null && res.code ===0) {
            this.getData();
            this.model= loaiModel.baseJson()
            this.theModal.hide();
        ...
        await this.$store.dispatch("loaiStore/create", this.model).then((res) => {
          if (res != null && res.code ===0) {
            this.getData();
            this.model= loaiModel.baseJson()
            this.theModal.hide();
