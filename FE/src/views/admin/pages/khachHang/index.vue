...
export default {
  ...
  methods: {
    async getData() {
      ...
      await this.$store.dispatch("khachHangStore/getPagingParams", params ).then(res => {
    ...
    async handleGetInfo(id) {
      await this.$store.dispatch("khachHangStore/getById", {id : id}).then((res) => {
        if (res != null && res.code ===0) {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("khachHangStore/delete", { 'id': this.model.id }).then((res) => {
          if (res != null && res.code ===0) {
    ...
    async handleSubmit() {
      if (
        ...
        await this.$store.dispatch("khachHangStore/update", this.model).then((res) => {
          if (res != null && res.code ===0) {
            this.getData();
            this.model= khachHangModel.baseJson()
            this.theModal.hide();
