...
namespace badmintion.Controllers
{
    ...
    public class OdersController : DefaultReposityController<Order>
    {
        ...
        [HttpPost]
        [Route("get-paging-params-core-status2")]
        public  async Task<IActionResult> GetPagingCore2([FromBody] PagingParamDefault model)
        {
            try
            {
                var response = await _service.GetPagingCoreStatus2(model);
                return Ok(
                        new ResultMessageResponse()
                            .WithData(response)
                            .WithCode(DefaultCode.SUCCESS)
        ...
        public  async Task<IActionResult> GetPagingCore3([FromBody] PagingParamDefault model)
        {
            try
            {
                var response = await _service.GetPagingCoreStatus3(model);
                return Ok(
                        new ResultMessageResponse()
                            .WithData(response)
                            .WithCode(DefaultCode.SUCCESS)
        ...
        public override async Task<IActionResult> GetById([FromBody] IdFromBodyModel model)
        {
            try
            {
                var response = await _service.GetIdCore(model.Id);
                return Ok(
                        new ResultMessageResponse()
                            .WithData(response)
                            .WithCode(DefaultCode.SUCCESS)
